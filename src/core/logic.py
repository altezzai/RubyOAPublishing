    ...
    if display_group == "submission":
        group_of_settings = [
            ...
            },
            {
                ...
                "object": setting_handler.get_setting(
                    "general", "user_automatically_author", journal
                ),
            },
            {
...
def is_citizen_request(request):
    return request.headers.get("X-Frontend-Origin") == "citizenScience"


def is_knowledge_request(request):
    return request.headers.get("X-Frontend-Origin") == "knowledgeCommons"


def is_create_staff_request(request):
    return is_knowledge_request(request) and request.POST.get("osp_role")


def get_user_osp_role(request):
    jwt_payload = decode_jwt_token(request)
    return jwt_payload.get("role")


def mutate_user_role(user, role):
    if role in models.OSPRoles.values():
        user.is_active = True
        user.osp_role = role
        user.save()
        return user
