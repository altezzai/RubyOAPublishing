...
from knowledge_connector.models import KC_Submission
...
def send_article_to_knowledge_commons(request, article, **kwargs):
    try:
        existing_submission = (
            KC_Submission.objects.using("knowledge")
            .filter(Q(doi=article.get_doi()) | Q(title=article.title))
            .first()
        )

        if existing_submission:
            print(
                f"Article already exists in knowledge commons: {existing_submission.id}"
            )
            return existing_submission

        authors_id_list = list(article.authors.all().values_list("id", flat=True))
        ...
        submission_data = {
            ...
            "title": article.title,
            "file_path": file_path,
            "authors": json.dumps(authors_id_list),
        ...
        article_submission = KC_Submission.objects.using("knowledge").create(
            **submission_data
        )
        ...
        print(f"Error submitting article to knowledge commons: {e}")
        return None
